...
void draw_thread(void *args)
{
    ...
    static unsigned char **LL, **VV;	
                                /* neighbor lists for StartP and VoronjP */
...
/* Subroutines ///////////////////////////////////////////////// */
    ...
    inline INT Sleep( INT dur ) {
	/* sleeps only if the saver stays active */
    ...
    void DataCreation(){
	/* creates np random points and 4 fixed corner p. */
	INT i;
    ...
    inline double SqDist( POINTL p1, POINTL p2 ){
	/* square dist of p1 - p2 */
	...
	/* adds the index of the Voronjpoint to the voronjpoint-lists of */
	/* each of the three points that constitute the triangle */
	...
	if( !found ){	/* current VoronjPoint is a new one */
	    ...
	    ch = itest;
	    if( strchr( VV[i], ch ) == NULL ) strncat( VV[i], &ch, 1);	
	    if( strchr( VV[k], ch ) == NULL ) strncat( VV[k], &ch, 1);	
	    if( strchr( VV[j], ch ) == NULL ) strncat( VV[j], &ch, 1);
	}
    }
	...
	/* 20 is enough for the arrays because the VV-strings only */
	/* have size 20 */
	...
	for( i = 1; i <= np; i++){
	    ...
	    Sort( 0, l-1 );
	    /* reorder the elem. in VV[i] => adjust them according to angle */
	    for(j = 0; j < l; j++)
		Satz[j] = VV[i][io[j]];
	    for(j = 0; j < l; j++)
		VV[i][j] = Satz[j];
	...
	for( i = 1; i <= np; i++){	/* from LL[1] to LL[np] */
		...
		k = LL[i][j];	/* i and k should be neighbors, that means */
		/* they have to share at least one Voronj point */
		test = 0;
		q = 0;
		size = strlen( VV[k] );
		while( q < size && !test ){
		...
		if( !test ){
		    /* i and k don't share any visible Voronj points */
		    strcpy( &LL[i][j], &LL[i][j+1] );
				/* delete k from i's neighborlist */
		    m = 0;
		    /* search i in k's neighborlist: */
		    while( LL[k][m] != i ) m++;	
	...
	*here = from;
	...
	do{
		...
		while ( pos != testlen ){
		    ...
		    testlen--;
		    while( Test[pos] != Col && pos != testlen) pos++;
		    /* search again */
		}
		pos = 0;	/* to prevent an accidental interrupt */
	    }
	} while( Test[0] != CLR_BLACK && pos != testlen );
	/* while there are still colors in Test */ 
	/* and a color doesn't appear only once */
	if( pos == testlen ){	/* found a color that appears only once ... */
	...
	*Lauf = '\0';
	...
	while( ColorIsAt( 1, np, CLR_CYAN, &m ) ){
	    ...
	    if ( strstr( Hist, Wort ) != NULL ){ /* endless loop: Hist contains Wort */
...
/* End of Subroutines ///////////////////////////////////////// */
    ...
    if( !(Queue = (unsigned char*)malloc( 256 * sizeof(unsigned char))) ) 
	goto end;
    if(!(LL = (unsigned char**)malloc( (npp+1) * sizeof(unsigned char*))) ) 
	goto end;
    if(!(VV = (unsigned char**)malloc( (npp+1) * sizeof(unsigned char*))) ) 
	goto end;
    for( np = 0; np <= npp && !stop_draw_thread; np++ ){
	if(!(LL[np] = (unsigned char*)malloc( 20 * sizeof(unsigned char))) ) 
	    goto end;
	if(!(VV[np] = (unsigned char*)malloc( 20 * sizeof(unsigned char))) ) 
	    goto end;
    }	/* I use np because it is an existing and here unused variable */
    if( !(StartP = (POINTL*)malloc( (npp+1) * sizeof(POINTL))) ) 
	goto end;
    if( !(VoronjP = (POINTL*)malloc( npp * 2 * sizeof(POINTL))) ) 
	goto end;
    if( !(Farbe = (INT*)malloc( (npp+1) * sizeof(INT))) ) 
	goto end;
