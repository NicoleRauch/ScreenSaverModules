...
void	draw_thread(void *args)
{
   ...
   POINTL l_b = { 0, 0 },
	...
	for( j = 0; j < 16; j++){
		step = j / 4;
		i = j % 4;
		BitmapPoints[j][2].x = step * screenSizeX / 4 - 1;
		BitmapPoints[j][2].y = i * screenSizeY / 4 - 1;
		BitmapPoints[j][3].x = (step + 1) * screenSizeX / 4 - 1;
		BitmapPoints[j][3].y = (i + 1) * screenSizeY / 4 - 1;
	}
	...
	while(!stop_draw_thread){
		if( configuration_data.drunken ){
			for( j = 0; j < 16; j++ ){
				...
				switch ( step ){
					case 0:  BitmapPoints[j][0].x = BitmapPoints[j][2].x;
								BitmapPoints[j][0].y = BitmapPoints[j][2].y + 1;
								BitmapPoints[j][1].x = BitmapPoints[j][3].x;
								BitmapPoints[j][1].y = BitmapPoints[j][3].y + 1;
								break;
					case 1: 	BitmapPoints[j][0].x =  BitmapPoints[j][2].x + 1;
								BitmapPoints[j][0].y =  BitmapPoints[j][2].y + 1;
								BitmapPoints[j][1].x =  BitmapPoints[j][3].x + 1;
								BitmapPoints[j][1].y =  BitmapPoints[j][3].y + 1;
								break;						 
					case 2: 	BitmapPoints[j][0].x =  BitmapPoints[j][2].x + 1;
								BitmapPoints[j][0].y =  BitmapPoints[j][2].y;
								BitmapPoints[j][1].x =  BitmapPoints[j][3].x + 1;
								BitmapPoints[j][1].y =  BitmapPoints[j][3].y;
								break;							
					case 3: 	BitmapPoints[j][0].x =  BitmapPoints[j][2].x + 1;
								BitmapPoints[j][0].y =  BitmapPoints[j][2].y - 1;
								BitmapPoints[j][1].x =  BitmapPoints[j][3].x + 1;
								BitmapPoints[j][1].y =  BitmapPoints[j][3].y - 1;
								break;							
					case 4: 	BitmapPoints[j][0].x =  BitmapPoints[j][2].x;
								BitmapPoints[j][0].y =  BitmapPoints[j][2].y - 1;
								BitmapPoints[j][1].x =  BitmapPoints[j][3].x;
								BitmapPoints[j][1].y =  BitmapPoints[j][3].y - 1;
								break;							
					case 5: 	BitmapPoints[j][0].x =  BitmapPoints[j][2].x - 1;
								BitmapPoints[j][0].y =  BitmapPoints[j][2].y - 1;
								BitmapPoints[j][1].x =  BitmapPoints[j][3].x - 1;
								BitmapPoints[j][1].y =  BitmapPoints[j][3].y - 1;
								break;							
					case 6: 	BitmapPoints[j][0].x =  BitmapPoints[j][2].x - 1;
								BitmapPoints[j][0].y =  BitmapPoints[j][2].y;
								BitmapPoints[j][1].x =  BitmapPoints[j][3].x - 1;
								BitmapPoints[j][1].y =  BitmapPoints[j][3].y;
								break;							
					case 7: 	BitmapPoints[j][0].x =  BitmapPoints[j][2].x - 1;
								BitmapPoints[j][0].y =  BitmapPoints[j][2].y + 1;
								BitmapPoints[j][1].x =  BitmapPoints[j][3].x - 1;
								BitmapPoints[j][1].y =  BitmapPoints[j][3].y + 1;
								break;
		...
		for( i = 1; i < 5; i++){
			for( j = 0; j < 16; j++ ){
				line();
				GpiBitBlt( hps, hps, 3, BitmapPoints[j], ROP_SRCCOPY, BBO_IGNORE );	
				if( stop_draw_thread){
					 i = 100;
					 j = 100;
				}
