...
{
/*
	$$$$$ replace or change the following example code $$$$$
	int	i, j;
	POINTL	pt_corner[4];
...
//	HMQ	drawingthread_hmq = WinCreateMsgQueue(drawingthread_hab, 0);
	...
	pt = alloca(configuration_data.count * sizeof(POINTL));

	// set the corner coordinates
	pt_corner[0].x = 0;
	pt_corner[0].y = 0;
	pt_corner[1].x = 0;
	pt_corner[1].y = screenSizeY-1;
	pt_corner[2].x = screenSizeX-1;
	pt_corner[2].y = 0;
	pt_corner[3].x = screenSizeX-1;
	pt_corner[3].y = screenSizeY-1;

	while(!stop_draw_thread){
		if(point_buffer_filled){
			// redraw old pyramid in black
			GpiSetColor(hps, CLR_BLACK);
			for(j=0;j<4;j++){
				GpiMove(hps, &pt[i]);
				GpiLine(hps, &pt_corner[j]);
			}
		}

		// select random color
		GpiSetColor(hps, ((unsigned)rand()) % 16);

		// select random point and store it in buffer
		pt[i].x = ((unsigned)rand()) % screenSizeX;
		pt[i].y = ((unsigned)rand()) % screenSizeY;
