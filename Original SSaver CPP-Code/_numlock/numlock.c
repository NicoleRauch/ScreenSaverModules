...
/*
	...
	$$$$$ for example $$$$$
*/
#define BLINK_RAND_FLASH	0
#define BLINK_RANDOM 		1
#define BLINK_IN_LINE 		2
#define BLINK_WALK_ONE		3
#define BLINK_SYNC			4
...
/*
  ...
  */
	hwndLbox = WinWindowFromID( hwnd, IDC_COMBO );
	r1 = WinInsertLboxItem( hwndLbox, BLINK_RAND_FLASH, (PSZ)"Random Flash" );
	r1 = WinInsertLboxItem( hwndLbox, BLINK_RANDOM, (PSZ)"Real Random" );
	r1 = WinInsertLboxItem( hwndLbox, BLINK_IN_LINE, (PSZ)"In Line" );
	r1 = WinInsertLboxItem( hwndLbox, BLINK_WALK_ONE, (PSZ)"Walking One" );
	r1 = WinInsertLboxItem( hwndLbox, BLINK_SYNC, (PSZ)"Synchronous Blink" );
	WinSendMsg( hwndLbox, LM_SELECTITEM, MPFROMLONG(configuration_data.blinkmode), (MPARAM)TRUE );
...
/*
  ...
  */
	    configuration_data.blinkmode = WinQueryLboxSelectedItem( hwndLbox );
...
/*
  ...
  */
	    configuration_data.blinkmode = BLINK_WALK_ONE;
	    write_profile_data();
	}
...
void draw_thread(void *args)
{
    ...
    INT i = -1;
    BYTE KeyStateTable[257], OldKeyStateTable[257], Index[3];
    EXCEPTIONREGISTRATIONRECORD xcpthand = { (PEXCEPTIONREGISTRATIONRECORD)0, 
					     GPFHandler };

    DosSetExceptionHandler(&xcpthand) ;

    Index[0] = VK_CAPSLOCK + 1;	/* Caps Lock */
    Index[1] = VK_SCRLLOCK + 1;	/* Scroll Lock */
    Index[2] = VK_NUMLOCK + 1;		/* Num Lock */
	
    if(!WinSetKeyboardStateTable( HWND_DESKTOP, &OldKeyStateTable[1],
				  FALSE ))
	DosBeep(500, 150 );
    if(!WinSetKeyboardStateTable( HWND_DESKTOP, &KeyStateTable[1], FALSE ))
	DosBeep(500, 150 );
    KeyStateTable[Index[0]] = 0;
    KeyStateTable[Index[1]] = 0;
    KeyStateTable[Index[2]] = 0;
    WinSetKeyboardStateTable( HWND_DESKTOP, &KeyStateTable[1], TRUE );
    /* reset all lights to OFF	*/
	
    WinFillRect( hps, &ScreenRect, CLR_BLACK );
	
    while(!stop_draw_thread){
	switch( configuration_data.blinkmode ) { /* adjust KeyStateTable */
	case BLINK_RAND_FLASH:
	    i = rand() % 3;
	    KeyStateTable[Index[i]] ^= 1;
	    WinSetKeyboardStateTable( HWND_DESKTOP, &KeyStateTable[1], 
				      TRUE );
	    DosSleep( 
		50 + (4 - configuration_data.animation_speed ) * 10 );
	    /* turn the light on for a short while and then off again */
	    KeyStateTable[Index[i]] ^= 1;
	    WinSetKeyboardStateTable( HWND_DESKTOP, &KeyStateTable[1], 
				      TRUE );
	    DosSleep( 100 );
	    break;
	case BLINK_RANDOM:
	    KeyStateTable[Index[rand() % 3]] ^= 1;	
	    WinSetKeyboardStateTable( HWND_DESKTOP, &KeyStateTable[1], 
				      TRUE );
	    DosSleep( 
		200 + (4 - configuration_data.animation_speed ) * 20 );
	    break;
	case BLINK_IN_LINE:
	    i++;
	    i %= 3;
	...
	case BLINK_WALK_ONE:
	    ...
	    WinSetKeyboardStateTable( HWND_DESKTOP, &KeyStateTable[1], 
				      TRUE );
	    DosSleep( 
		300 + (4 - configuration_data.animation_speed ) * 30 );
	    /* turn the light on for a short while and then off again */
	    KeyStateTable[Index[i]] ^= 1;
	    WinSetKeyboardStateTable( HWND_DESKTOP, &KeyStateTable[1], 
				      TRUE );
	    break;
	case BLINK_SYNC:	
	    KeyStateTable[Index[0]] ^= 1;
	    KeyStateTable[Index[1]] ^= 1;
	    KeyStateTable[Index[2]] ^= 1;
	    WinSetKeyboardStateTable( HWND_DESKTOP, &KeyStateTable[1], 
				      TRUE );
	    DosSleep( 
		200 + (4 - configuration_data.animation_speed ) * 20 );
    ...
    WinSetKeyboardStateTable( HWND_DESKTOP, &OldKeyStateTable[1], TRUE );
    /* reset KeyStateTable */
